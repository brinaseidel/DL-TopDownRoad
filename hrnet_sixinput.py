...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.bn1_3 = BatchNorm2d(64, momentum=BN_MOMENTUM)
        self.bn1_4 = BatchNorm2d(64, momentum=BN_MOMENTUM)
        self.bn1_5 = BatchNorm2d(64, momentum=BN_MOMENTUM)
    ...
    def forward(self, x):
        ...
        # Convolve the six imput images seperately
        x0 = self.conv1_0(x[:, 0, :, :])
        x1 = self.conv1_1(x[:, 1, :, :])
        x2 = self.conv1_2(x[:, 2, :, :])
        x3 = self.conv1_3(x[:, 3, :, :])
        ...
        x5 = self.bn1_5(x5)
        x = self.relu(x) 
        #print("one image after conv1 and bn1: ", x0.shape)
        
        # Tile the six imput images
        # CAM_FRONT_LEFT, CAM_FRONT, CAM_FRONT_RIGHT, CAM_BACK_LEFT, CAM_BACK, CAM_BACK_RIGHT
        x = torch.cat((torch.cat((x0,  x1,  x2), dim=3),
                       torch.cat((x3,  x4,  x5), dim=3)), dim=2)
        #print("after tiling: ", x.shape) 
